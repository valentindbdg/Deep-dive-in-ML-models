...
class LinearRegression(object):
    ...
    def feature_transform(self, X):
        ...
        #X_transformed = None
        #X_transformed = np.append(X, bias_term, axis=1)
        bias = np.ones((X.shape[0], 1))        
        X_transformed = np.concatenate((X, bias), axis=1)
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def loss(self, X, y=None):
        ...
        #loss = 0
        #for i in range(N):
        #    loss = loss + 1/(2*N)* sum((prediction[i] - y[i])**2)
        #loss = np.sum(np.power((prediction-y), 2)) / (2*N)
        a=np.power((prediction-y),2)
        b=np.sum(a)
        loss = b/(2*N)
        #############################################################################
        #                              END OF YOUR CODE                             #
        ...
        #############################################################################
        grads['W'] = (X.T).dot((prediction-y))/N
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
    ...
    def train(self, X, y, X_val=None, y_val=None,learning_rate=1e-3, learning_rate_decay=0.95,
        ...
        for it in range(num_iters):
            ...
            #    X_batch[i] = X[np.random.choice(num_train[i], batch_size[i])]
             #   y_batch[i] = y[np.random.choice(num_train[i], batch_size[i])]
            ...
            #for i in range(batch_size):
             #   X_batch[i] = X[minibatch[i]]
              #  y_batch[i] = y[minibatch[i]]
            xyrandom=np.random.choice(num_train, batch_size, replace=False )
            X_batch = X[xyrandom]
            y_batch = y[xyrandom]
            #########################################################################
            #                             END OF YOUR CODE                          #
